...
from Zelda import BLACKLIST_CHAT, BOTLOG_CHATID, CLEAN_WELCOME
from Zelda import CMD_HANDLER as cmd
from Zelda import CMD_HELP, LOGS, bot
...
async def welcome_to_chat(event):
    try:
        from Zelda.modules.sql_helper.welcome_sql import (
...
async def save_welcome(event):
    ...
    try:
        from Zelda.modules.sql_helper.welcome_sql import add_welcome_setting
    except AttributeError:
...
async def show_welcome(event):
    try:
        from Zelda.modules.sql_helper.welcome_sql import get_current_welcome_settings
    except AttributeError:
...
async def del_welcome(event):
    try:
        from Zelda.modules.sql_helper.welcome_sql import rm_welcome_setting
    except AttributeError:
        return await event.edit("`Running on Non-SQL mode!`")
